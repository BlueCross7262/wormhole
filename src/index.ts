...
// 기동 pull(best-effort) → StdioServerTransport connect.
...
async function main(): Promise<void> {
  ...
  // 8) 기동 pull — MCP 서버 시작 시 무조건 1회 수행.
  try {
    const result = await engine.pull();
    logger.info(`시작 pull 완료: applied=${result.applied.length} removed=${result.removed.length} conflicts=${result.conflicts.length}`);
  } catch (err) {
    logger.warn(`시작 pull 실패(무시하고 계속): ${String((err as Error).message)}`);
  }

  // 9) graceful shutdown — 진행 작업 정리 후 종료.
