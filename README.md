...
```
   ┌──────────────────────────────┐
   │ Claude Code 슬래시 커맨드      │
   │ /wormhole-status /-sync 등    │
   │     │ shell out               │
   │     ▼                         │
   │ wormhole CLI (dist/cli.mjs)  │
   │ argv 디스패처 (단발 실행)     │
   ├──────────────────────────────┤
   │ status/resolve/sync/doctor    │
...
| 모듈 | 책임 |
|---|---|
| `src/cli.ts` | CLI argv 디스패처 진입점 (status/resolve/sync/doctor), JSON→stdout |
| `src/doctor.ts` | `runDoctor` — 6단계 진단 (config/WebDAV/passphrase/vault 정합/vault 상태/transport 보안) |
...
| 커맨드 | 동작 |
|---|---|
| `/wormhole-status` | 추가/수정/삭제/원격변경/충돌/수렴 요약 (변경 없음) |
| `/wormhole-resolve` | 충돌을 명시적으로 해소 |
| `/wormhole-sync` | **원샷** — pull → 충돌 시 자동 해소(`preserve-both`) → push |
| `/wormhole-doctor` | 설정·WebDAV 연결·passphrase 정합·vault 상태 6단계 진단 (읽기전용) |

일상적으로는 `/wormhole-sync` 하나면 충분하다. pull 후 충돌이 있으면 기본 정책 (`preserve-both`) 으로 해소한 뒤 push 까지 한 번에 처리한다 (커맨드 상세는 **§7 슬래시 커맨드** 참조). 연결·설정 문제는 `/wormhole-doctor` 로 진단한다.
...
| `wormhole_doctor` | 읽기 전용 | 자율 호출 가능 | 설정·WebDAV·passphrase·vault 6단계 진단 (변경 없음) |

> **네이밍 규칙**: 슬래시 커맨드는 hyphen(`wormhole-status` / `-resolve` / `-sync` / `-doctor` / `-setup`), MCP 도구 id 는 underscore(`wormhole_status` / `_resolve` / `_sync` / `_doctor`) 를 사용한다. 생태계 관행(MCP = snake_case)과 Claude Code 슬래시 커맨드 네임스페이싱 제약을 각각 반영한 의도적 분리다.

### confirm-gate 안전 모델

**읽기 전용 도구** (`wormhole_status`, `wormhole_doctor`) 는 로컬·원격을 변경하지 않으므로 Claude 가 자율적으로 호출할 수 있다.
...
>
> 훅·스테이터스라인 스크립트 파일은 동기화되지만, `settings.json` 의 훅 연결(`hooks.*`)과 `statusLine.command` 는 **머신 로컬** 로 유지된다.
> 이유: `hooks.*.command` 와 `statusLine.command` 에는 Windows `C:\Program Files\nodejs\node.exe` 같은 OS·머신 고유 인터프리터 절대경로가 들어가며, `${HOME}` 토큰화 범위 밖이라 그대로 동기화하면 타 머신에서 경로가 깨진다.
...
node dist/cli.mjs doctor         # 설정·WebDAV·passphrase 6단계 진단
...
### `/wormhole-resolve` (`resolve [--policy P] [--keys k1,k2] [--dry-run]`)
...
- `--policy manual` 은 **금지** 다 (sync 안에서는 자동 해소가 전제이므로 에러 — 수동 해소가 필요하면 `/wormhole-resolve` 를 따로 쓴다).
- 일상 동기화는 이 커맨드 하나면 충분하다.

### `/wormhole-doctor` (`doctor`)

**진단 전용 (읽기전용)** — 설정·WebDAV 연결·passphrase·vault 정합 6단계를 점검하고 각 항목의 ✓/✗/⚠ 결과를 반환한다. 로컬·원격 데이터를 변경하지 않는다.

- **Check 1 — config**: `~/.wormhole/.env` 존재, 필수 키(WEBDAV_URL/USER/PASS), `config.json` zod 스키마 유효성.
- **Check 2 — WebDAV 연결·인증**: PROPFIND 성공 여부 (401 = 인증 실패, 그 외 = 연결 실패).
- **Check 3 — passphrase 소스**: env / 0600 파일 / keychain 어디에서 passphrase 를 읽었는지 보고.
- **Check 4 — passphrase↔vault 정합**: 원격 `keyparams.json` sentinel 복호로 passphrase 일치 여부 검증 (vault 없으면 ⚠ 미초기화).
- **Check 5 — vault 상태**: keyparams/manifest 존재, `manifestGeneration`, entries 수 보고.
- **Check 6 — transport 보안**: 평문 `http://` (localhost 제외) 사용 시 ⚠ 경고.
- 하나라도 ✗(fail) 이면 exit code 1 로 종료 (CLI). ⚠(warn) 는 ok 유지.
- 서버 부팅 실패 시 MCP 도구 `wormhole_doctor` 는 노출되지 않는다 — 이 경우 CLI `wormhole doctor` 로 진단한다.
...
대개 `/wormhole-sync` 하나면 된다. pull → 충돌 시 자동 해소(`preserve-both`) → push 를 한 번에 처리한다. 세밀한 제어가 필요하면 `/wormhole-status` 로 상태를 확인한 뒤 `/wormhole-resolve` 로 충돌을 해소하고 다시 `/wormhole-sync` 를 호출한다 (커맨드 상세는 **§7 슬래시 커맨드** 참조). 연결·설정 문제는 `/wormhole-doctor` 로 진단한다.
...
- `settings.json` 은 키 단위 3-way 머지로 동기화된다. `settings.local.json` 은 동기화에서 제외된다 (기기 로컬 오버라이드).
...
- `/wormhole-doctor` (또는 `wormhole doctor`) 로 설정·WebDAV·passphrase 6단계를 한 번에 점검한다.

### WebDAV 연결 오류

- `/wormhole-doctor` 의 Check 2 결과를 확인한다 — 401 이면 자격증명, 그 외면 URL·네트워크 문제.
