...
| `remote.remoteBaseDir` | 원격 루트 경로 | `/wormhole` |
...
```bash
export WORMHOLE_WEBDAV_URL="https://your-webdav-server.example.com"
export WORMHOLE_WEBDAV_USER="your-webdav-username"
export WORMHOLE_WEBDAV_PASS="your-webdav-password"
export WORMHOLE_PASSPHRASE="your-strong-passphrase"
export WORMHOLE_LOG_LEVEL="info"   # debug | info | warn | error
```
...
```json
{
  "mcpServers": {
    "wormhole": {
      ...
      "env": {
        "WORMHOLE_CONFIG": "/absolute/path/to/config.json",
        "WORMHOLE_WEBDAV_PASS": "your-webdav-password",
        "WORMHOLE_PASSPHRASE": "",
...
> `args` 와 `WORMHOLE_CONFIG` 는 반드시 절대경로로 지정한다. Windows 는 `C:/Users/user/...`, WSL2 는 `/home/user/...` 형식이다.
> `WORMHOLE_PASSPHRASE` 를 평문으로 넣는 대신 0600 파일이나 keychain 사용을 권장한다 (4번 참고).
...
Restart=on-failure
RestartSec=5
Environment=WORMHOLE_CONFIG=/home/user/.wormhole/config.json
Environment=WORMHOLE_PASSPHRASE_FILE=/home/user/.wormhole/passphrase
Environment=WORMHOLE_WEBDAV_PASS=your-webdav-password
...
<dict>
  ...
  <dict>
    <key>WORMHOLE_CONFIG</key>
    <string>/Users/user/.wormhole/config.json</string>
...
- 환경변수 (`WORMHOLE_CONFIG`, `WORMHOLE_PASSPHRASE_FILE` 등) 는 작업의 환경 또는 사용자 환경변수로 주입한다.
...
- `dist/index.js` 경로와 `WORMHOLE_CONFIG` 가 절대경로인지 확인한다.
