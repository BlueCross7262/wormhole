...
// CGW-02(pull 게이트), CGW-07(sync 충돌 자동해소), CGW-08(sync 무충돌 실적용),
...
// 충돌 상태 만들기: A v1 push → B pull → A v2 push → B 로컬 v3.
...
// ── CGW-02: pull confirm 게이트 ──
test("CGW-02: pull confirm 게이트 실와이어", async (t) => {
  const { homeB, a, b } = await twoMachines(t, { aFiles: { ".claude/CLAUDE.md": "x\n" } });
  await a.callTool("wormhole_push", { confirm: true });

  const prev = sc(await b.callTool("wormhole_pull")); // 생략
  assert.equal(prev.dryRun, true, "dryRun:true");
  assert.ok(typeof prev.note === "string", "note 존재");
  assert.equal(exists(homeB, ".claude/CLAUDE.md"), false, "미리보기 후 로컬 미생성");

  const applied = sc(await b.callTool("wormhole_pull", { confirm: true }));
  assert.equal(applied.dryRun, false, "confirm:true → 적용");
...
test("CGW-08: sync 무충돌 → resolve 부재", async (t) => {
  ...
  await a.callTool("wormhole_push", { confirm: true }); // 원격만 변경, B 충돌 없음
...
test("SMR-01: settings 3-way 로컬키 격리", async (t) => {
  ...
  await a.callTool("wormhole_push", { confirm: true });
  await b.callTool("wormhole_pull", { confirm: true });               // B base = {theme:dark}
...
test("SMR-08: 발산(no-base) → conflict, 로컬 보존, 미적용", async (t) => {
  ...
  await a.callTool("wormhole_push", { confirm: true });
  const pull = sc(await b.callTool("wormhole_pull", { confirm: true }));
...
test("TMB-03: 독립 동일콘텐츠 → converged 분류", async (t) => {
  ...
  await a.callTool("wormhole_push", { confirm: true }); // 원격 = same
...
test("ELC-04: 두 머신 동시 push 무손상", async (t) => {
  ...
  // 서로 다른 키를 동시 push(같은 원격 lock/manifest CAS 경합).
  const [pa, pb] = await Promise.all([
    a.callTool("wormhole_push", { confirm: true }),
    b.callTool("wormhole_push", { confirm: true }),
  ]);
  // CAS/락 재시도로 둘 다 성공해야(혹은 최소 무크래시 + 후속 성공).
  assert.equal(parseToolResult(pa).isError, false, "A push 무에러");
  assert.equal(parseToolResult(pb).isError, false, "B push 무에러");
