...
// passphrase 보유 → 원격 키 파생해 manifest 복호·재암호화)하고, 머신B wormhole_pull 이
...
test("F-ENGINE-24: 악성 원격 manifest 키 → pull 경로탈출 거부", async (t) => {
  ...
  await a.initialize();
  await a.callTool("wormhole_push", { confirm: true }); // vault + manifest + blob 부트스트랩
  await a.close();
  ...
  assert.equal(pull.isError, false, "pull 자체는 graceful(크래시 아님)");
  assert.ok(pull.structured.applied.includes(".claude/CLAUDE.md"), "유효 키 적용");
  for (const k of Object.values(EVIL)) {
    assert.ok(!pull.structured.applied.includes(k), `악성 키 미적용: ${k}`);
  }
...
test("SMR-09: 악성 원격 settings blob(__proto__) → pull Object.prototype 무오염", async (t) => {
  ...
  const pull = parseToolResult(await b.callTool("wormhole_pull", { confirm: true }));
  assert.equal(pull.isError, false, "pull graceful");
